...
and the camera follows. The first NPC slice is now live as well:
`MapObjectManager` creates flag-visible `ObjectEvent`s, resolves and caches
their ov01 move models, ticks their existing movement/animation machines,
and publishes their billboards (Mom is present on 1F). Visual checks against
retail frames: bedroom and house 1F match pixel-for-pixel on the player; New
Bark's framing matches after the height fix; house 1F's NPC-inclusive frame
is pinned against the local SHA-1-gated ROM.
Not yet in this checkpoint: NPC autonomous movement or Mom's scripted
scene, dialogue/script host, the bag/party menus and the touch bottom screen
(black), lighting applied to the renderer,
...
`cargo run -p apricorn-desktop -- --rom hg_usa.nds` — loads and updates the
gitignored `apricorn.sav`; choose CONTINUE, then walk (arrows) around the
bedroom, down the stairs, out the front door and around New Bark Town.
The desktop checkpoints the retail-format card during field movement and on
normal exit. **Keep this persistent save workflow for all future manual
regression runs** so renderer iteration never requires recreating a character.
Use `--save <path>` for another disposable card copy and `--no-save` only for
an intentionally fresh deterministic run. Never use the ROM path as the save
target. Headless equivalents:
...
1. **NPC objects + script host** — `MapObjectManager` from each map's
   ...
   - [x] Live `MapObjectManager`: flag-filtered `ObjectEvent` creation,
         full interaction metadata, ov01 sprite→model loading with shared
         texture strips, ground-height placement, SysTask movement/animation
         ticks, pause control, id lookup and `FieldFrame` billboards.
   - [ ] Autonomous NPC movement types and their LCRNG/arm-runner checks.
   - [ ] A-button targeting plus the live field `ScriptHost`.
   - [ ] Map-load/init scripts, BG events and coordinate triggers.
...
- [ ] Map engine: HGSS's map/BG layers, collision, warp/door transitions,
      ...
      - [ ] Complete the DS-exact 3D core. The landed fixed-point camera,
            viewport quantization, preserved triangle/quad identity, integer
            scanlines, perspective texel interpolation and field crack
            regression remove the field-critical camera shimmer, but do not
            yet justify a whole-core pixel-exact claim.
            - [ ] Homogeneous six-plane clipping plus hardware face-culling
                  and polygon ordering.
            - [ ] GX normal transform and four-light material evaluation,
                  including diffuse/ambient/specular/emission, toon table and
                  shininess table.
            - [ ] Hardware Z/W depth selection and exact translucent
                  polygon-id/depth-update rules.
            - [ ] Shadow polygons and hardware-exact fog, edge marking and
                  antialias coverage resolve.
            - [ ] Preserve raw texture/palette formats to raster time and
                  compare every format, repeat/flip and transparent texel
                  against oracle output.
            - [ ] Gate exactness on contiguous raw 3D colour/alpha, depth and
                  polygon-id oracle sequences, including identical
                  frame-to-frame pixel-change masks. Static hashes prove
                  determinism only.
            For every renderer change, capture and personally inspect multiple
            consecutive frames in the bedroom/house (chairs, counter and Mom's
            table) **and** multiple outdoor positions/directions before asking
            the user to evaluate. melonDS software rendering of the pinned
            retail ROM is the practical oracle; original-hardware capture wins
            if the two ever disagree. No melonDS GPL code may be copied or
            translated into this MIT implementation.
            Progress audit (2026-09-12): clipping, culling, light evaluation,
            Z/W selection, shadow stencil, fog/edge/AA and native sampling
            paths now have implementation and synthetic tests. Checkboxes
            remain open until oracle gates pass; feature presence is not
            proof of exactness. Retained model-local GX vertices and matrix
            concatenation fix a large class of depth/texture drift. Live
            retail field state is `DISP3DCNT=0x0039` (AA and edge marking ON),
            superseding the earlier AA-off assumption.
            `oracle/sequences/bedroom-down.tsv` is the repeatable ten-frame
            raw gate: currently 29–63 colour, 0–5 depth and 28–39 attribute
            differences per frame, zero coverage differences, and one
            mismatching pixel in the last transition mask. Missing ground
            shadow accounts for most initial colour differences. Do not
            update field goldens or claim full DS parity at this stage.
            Also outstanding: complete GX command/matrix stacks, per-command
            materials/texture matrices, DIRECT/COMP4x4, and complete indoor
            and outdoor raw oracle sequences. A validated common core is
            intended to prevent scene-specific pixel patches, not to promise
            that future missing assets/game-state bugs are impossible.
            Release New Bark raw-3D benchmark (120 measured frames):
            mean 4.113 ms, p95 5.191 ms, max 6.072 ms on this machine;
            below 16.714 ms. This excludes composition/presentation and
            therefore does not yet prove absence of desktop backlog.
