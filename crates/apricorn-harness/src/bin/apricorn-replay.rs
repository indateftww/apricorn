...
const USAGE: &str = "usage: apricorn-replay [--update] [--engine] [--rom ROM] [--shots FRAMES --shots-dir DIR] <case-dir>";
...
fn main() -> ExitCode {
    ...
    if engine && shots.is_some() {
        eprintln!("{USAGE}\n  --shots is an oracle feature; apricorn-run dumps engine frames (drop --engine)");
        return ExitCode::from(64);
    }
    ...
    match run(&case_dir, &rom, update, engine, shots.as_ref()) {
        Ok(code) => code,
        Err(message) => {
